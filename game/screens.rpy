...
screen building_detail(b):
    modal True
    # Return
    imagebutton:
        idle 'gui/bdetail/return.jpg'
        action Hide('building_detail')
    ...
    add 'images/figs/'+str(renpy.random.randint(1,1))+'.jpg':
        zoom 2.5
        xalign 0.5 yalign 0.65

    hbox:
        spacing 50
        xalign 0.5 yalign .85
        add 'gui/bdetail/total.jpg':
            zoom 1.5
        add 'gui/bdetail/infected.jpg':
            zoom 1.5
...
screen dashboard():
    ...
    grid 2 len(var.focused_buildings)/2:

        xoffset 50
        xspacing -50
        yspacing -1350*4/len(var.focused_buildings) + (len(var.focused_buildings)-4)/2*350

        yanchor 0

        style_prefix 'dashboard_cards'

        for i in range(0, len(var.focused_buildings)):
            fixed:
                button:
                    idle_background 'dashbaord_background_idle'
                    hover_background 'dashbaord_background_hover'
                    action Show('building_detail', b=var.focused_buildings[i])

                    has vbox
                    add "gui/buildings/"+str(var.focused_buildings[i])+'.png':
                        xoffset 7 yoffset 5
                        xalign .5
                        zoom 3.78
                    text str(var.toFullName[var.focused_buildings[i]]):
                        xalign .1 yalign .88
                        size 64
    fixed:
    ...
    # text _(str(len(var.focused_buildings))):
    #     align(.5,.5) size 88
...
screen dashboard_input():
    ...
    #     value action FieldInputValue('var.focused_buildings', returnable=True)
...
style dashboard_btn_fixed:
    xalign .5
    maximum(800, 106)

style dashboard_btn_text:
    align(.5, .5)

style dashboard_btn_button:
    align(.5, .3)
    maximum(800, 106)
    idle_background 'dashboard_addMorePics_btn'
    hover_background 'dashboard_addMorePics_btn'
...
screen calendar():
    ...
    for times in overall_schedule[date]:
        text str(times):
            xanchor 0.0 yanchor 0.0
            xalign 0.05 yalign 0.2*count
        vbox:
            yanchor 0.0 xanchor 1.0
            xalign 1.0 yalign 0.2*count
            for schedule in overall_schedule[date][times]:
                hbox:
    ...
    hbox:
        spacing 20
        ypos 0.12 xalign .5
        textbutton ("Mon") action SetVariable('date', '2/20/2022'):
            style_prefix 'calendar2'
        textbutton ("Tue") action SetVariable('date', '2/21/2022'):
            style_prefix 'calendar'
        textbutton ("Wed") action SetVariable('date', '2/22/2022'):
            style_prefix 'calendar'
        textbutton ("Thu") action SetVariable('date', '2/23/2022'):
            style_prefix 'calendar'
        textbutton ("Fri") action SetVariable('date', '2/24/2022'):
            style_prefix 'calendar'
        textbutton ("Sat") action SetVariable('date', '2/25/2022'):
            style_prefix 'calendar'
        textbutton ("Sun") action SetVariable('date', '2/26/2022'):
            style_prefix 'calendar'
...
style calendar_button_text:
    ...
    hover_color '#000000'
    idle_color '#000000'
    selected_color '#3318cc'

style calendar2_button_text:
    size 80
    hover_color '#b11b1b'
    idle_color '#b11b1b'
    selected_color '#3318cc'
...
screen bottom_bar(s):
    ...
    add 'gui/label/'+str(s)+'_label.png':
        zoom 1.5

    add 'gui/button/bottom_bar.png':
        zoom 1.5
        anchor(1.0, 1.0)
        align(1.0, 1.0)
    ...
    imagebutton:
        ...
        if renpy.get_screen('map'):
            ...
            action Hide(s, dissolve), Show('map')
    ...
    hbox:
        ...
        imagebutton:
            auto 'calenr_btn_%s'
            if renpy.get_screen('calendar'):
                action NullAction()
            else:
                action Hide(s, dissolve), Show('calendar')
        imagebutton:
            auto 'dashboard_btn_%s'
            if renpy.get_screen('dashboard'):
                action NullAction()
            else:
                action Hide(s, dissolve), Show('dashboard')
...
screen navigation():
    ...
    vbox:
        ...
        text _('Easy solution to'):
            style_prefix 'main_subtitle'

        text _('manage your money'):
            style_prefix 'main_subtitle'

        text _('Connect your wallet to your friends'):
            style_prefix 'main_subSubtitle'

        text _('family brand and experience'):
            style_prefix 'main_subSubtitle'
...
style main_subtitle_text:
    xalign .5
    size 92
    font 'Rubik-Medium.ttf'
    color '#ffffff'

style main_subSubtitle_text is main_subtitle_text

style main_subSubtitle_text:
    size 48

style main_btn_text is main_subSubtitle_text

style main_btn_fixed:
    maximum(486, 106)

style main_btn_text:
    align(.5, .5)

style main_btn_button:
    align(.5, .3)
    maximum(486, 106)
    idle_background 'login_btn_idle'
    hover_background 'login_btn_hover'
