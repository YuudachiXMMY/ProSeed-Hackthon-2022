...
public final class RatPoly {
    ...
    public RatTerm getTerm(int deg) {
        // {Inv: for each term, "term", in "terms", return "term" if its exponent equals to deg}
    ...
    public boolean isNaN() {
        // {Inv: for each term, "term", in "terms", return true if its coefficient is NaN}
        for(RatTerm term : terms) {
    ...
    /**
     * return a term copy of all terms of this
     *
     * @return copy of all terms in this
     */
    private List<RatTerm> copyTerms() {
    ...
    private static void scaleCoeff(List<RatTerm> lst, RatNum scalar) {
        // {Inv: for each RatTerm in lst, set it to a new RatTerm with coefficient multiplied by
        //      the scalar.}
    ...
    private static void incremExpt(List<RatTerm> lst, int degree) {
        if(degree != 0) {
            // {Inv: for each RatTerm in lst, set it to a new RatTerm with exponent added by
            //      the degree.}
            for(int i = 0; i < lst.size(); i++) {
    ...
    private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
        if(!newTerm.isZero()) {
            // {Inv: d_0, d_1, ... d_(i-1) which are degrees of each term > degree of newTerm,
            //       where d_i is the ith term in "terms"}
            for(int i = 0; i < lst.size(); i++) {
    ...
    public RatPoly add(RatPoly p) {
        ...
        List<RatTerm> r = p.copyTerms();
        // {Inv: r = p + t_0 + t_1 + ... + t_i-1, where t_i is the ith term in "terms"}
        for(RatTerm term : terms) {
            sortedInsert(r, term);
        }
        return new RatPoly(r);
    ...
    public RatPoly mul(RatPoly p) {
        ...
        // {Inv: r = t_0*p + t_1*p + ... + t_(i-1)*p, where t_i is the ith term in "terms"}
        for(RatTerm term : terms) {
            List<RatTerm> rp = p.copyTerms();
            scaleCoeff(rp, term.getCoeff());
            incremExpt(rp, term.getExpt());
            r = r.add(new RatPoly(rp));
        }
        r.checkRep();
        return r;
    }
    ...
    public RatPoly div(RatPoly p) {
        ...
        RatPoly v = new RatPoly();
        RatPoly r = new RatPoly(terms);
        // {Inv: this = p * v + r}
        while(r.terms.size() != 0 && r.degree() >= p.degree()) {
            RatTerm term = r.getTerm(r.degree()).div(p.getTerm(p.degree()));
            v = v.add(new RatPoly(term));
            r = r.sub(new RatPoly(term).mul(p));
        }
        v.checkRep();
        return v;
    }
    ...
    public RatPoly differentiate() {
        ...
        List<RatTerm> r = copyTerms();
        // {Inv: differentiate r_0, r_1, ... r_(i-1), where i is the ith term in "terms"}
        for(int i = 0; i < r.size(); i++) {
            if(r.get(i).getExpt() == 0) {
                r.remove(i);
                i--;
            } else {
                r.set(i, r.get(i).differentiate());
            }
        }
        return new RatPoly(r);
    }
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
        ...
        List<RatTerm> r = this.copyTerms();
        // {Inv: anti-antiderivative r_0, r_1, ... r_(i-1), where i is the ith term in "terms"}
        for(int i = 0; i < r.size(); i++)
            r.set(i, r.get(i).antiDifferentiate());
        if(!integrationConstant.equals(RatNum.ZERO)) {
            r.add(new RatTerm(integrationConstant, 0));
        }
        return new RatPoly(r);
    }
    ...
    public double integrate(double lowerBound, double upperBound) {
        ...
        RatPoly r = this.antiDifferentiate(new RatNum(0));
        r.checkRep();
    ...
    public double eval(double d) {
        ...
        double r = 0;
        // {Inv: r = t_0.eval(d) + t_1.eval(d) + ... + t_(i-1).eval(d), where t_i is
        //       the ith term in "terms"}
        for(RatTerm t : terms) {
            r += t.eval(d);
        }
        return r;
